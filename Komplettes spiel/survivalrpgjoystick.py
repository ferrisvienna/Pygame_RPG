...
#rebalance
    ...
    def GenerateLevel(self):
          ...
          for l in range(14):
            #for line in l:
                myline = ""
                for charpos in range(25):
                    char = random.choice(("g","d"))
                    myline += char
                level.append(myline)
...
class Jumptext(pygame.sprite.Sprite):
        ...
        def __init__(self, x=100,y=100, text="MissingNo...", color=(255,0,0),
            ...
            self.sparklecolor3=sparklecolor3
            
            self.font = pygame.font.SysFont('mono', self.fontsize, bold=True)
            self.fontWidth, self.fontHeight = self.font.size(self.text)
            self.image = self.font.render(text, True, color) # textcolor
            
            
            
            #self.image = Viewer.draw_
            self.image.set_colorkey((0,0,0)) # black transparent
            ...
            self.time = 0.0
            self.fragmentspeed = 20  # try out other factors !
            self.dx = 0
            self.dy = -self.fragmentspeed
        ...
        def update(self, seconds):
            ...
            self.rect.centery = round(self.y,0)
            if self.sparkling:
               self.color = random.choice((self.sparklecolor1,self.sparklecolor2,self.sparklecolor3))
...
#SpellSectionStart-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        def spell1(self):
            if self.alive:
              ...
              self.magic -= 400
              self.hitpoints -=1
              Jumptext(self.x,self.y,"Explode",(200,20,10), 30, True)
        ...
        def spell2(self):
            if self.alive:
              if self.magic >= 400:
                ...
                Shoot((self.x,self.y))
                self.magic -= 300
                Jumptext(self.x,self.y,"Shoot",(200,20,10), 30)
        ...
        def spell3(self):
            if self.alive:
              if self.magic>300:
                ...
                self.magic -= 300
            Jumptext(self.x,self.y,"Firestorm",(200,20,10), 30)
        ...
        def update(self, seconds):
            ...
            if self.stunned > 0:
                ...
                if self.stunned < 0:
                    self.stunned = 0
                if Game.food > 10 and Game.water > 10:
                    if self.hitpoints < self.hitpointsfull:
...
class Viewer(object):
     ...
     def __init__(self, width=0, height=0, fps=30):
        ...
        self.Actornumber = None
        Actortarget = None
        Jumptext.groups = self.allgroup, self.jumptextgroup
            #self.stats{Game.ACTOR_ATKDMG : "Dmg",Game.ACTOR_SPEED : "speed", Game.ACTOR_DEF : "Def"}
